...
//   Paradox Systems → direct PostgreSQL query
//   Company B       → CURL to their api/users.php
//   Company C       → CURL to their api/users.php
//
// Replace COMPANY_B_URL / COMPANY_C_URL with real partner URLs.
// ============================================================
session_start();
...
define('COMPANY_B_URL', 'https://company-b.onrender.com/api/users.php');
define('COMPANY_C_URL', 'https://company-c.onrender.com/api/users.php');
...
function fetch_local_users(): array {
    try {
        ...
        return ['company'=>'Paradox Systems','company_id'=>'paradox','color'=>'green','users'=>$stmt->fetchAll(),'error'=>null];
    } catch (Exception $e) {
...
function fetch_remote_users(string $url, string $company_name, string $color): array {
    ...
    $id = strtolower(str_replace(' ', '_', $company_name));
    if ($error || $code !== 200) {
        return ['company'=>$company_name,'company_id'=>$id,'color'=>$color,'users'=>[],'error'=>$error ?: "HTTP {$code}"];
    }
...
<head>
  ...
  <style>
    ...
    .admin-bar { position:fixed;top:0;left:0;right:0;z-index:1000;background:rgba(6,6,17,.97);
                 backdrop-filter:blur(20px);border-bottom:1px solid rgba(255,215,0,.2);padding:.75rem 0; }
    .admin-bar-inner { display:flex;align-items:center;justify-content:space-between;gap:1rem;
    ...
    .co-name  { font-family:var(--font-head);font-size:.85rem;color:var(--white);letter-spacing:.06em;margin-bottom:.35rem; }
    .co-count { font-family:var(--font-mono);font-size:2.2rem;line-height:1;margin-bottom:.3rem; }
    .co-count.green { color:var(--green); }
    .co-count.cyan  { color:var(--cyan); }
    .co-count.gold  { color:var(--gold); }
    ...
    .filter-bar input,.filter-bar select { background:var(--bg2);border:1px solid var(--border);
      color:var(--text);font-family:var(--font-mono);font-size:.75rem;padding:.55rem .9rem;outline:none; }
    .filter-bar input { flex:1;min-width:200px; }
    .count-badge { font-family:var(--font-mono);font-size:.7rem;color:var(--text-dim);margin-left:auto;white-space:nowrap; }
    ...
    th { font-family:var(--font-mono);font-size:.65rem;color:var(--text-dim);letter-spacing:.12em;
         text-transform:uppercase;padding:.85rem 1rem;text-align:left;border-bottom:1px solid var(--border);white-space:nowrap; }
    ...
    .role-badge { font-size:.6rem;letter-spacing:.08em;padding:.15rem .55rem;display:inline-block;border:1px solid; }
    ...
    @media(max-width:800px){ .company-cards{ grid-template-columns:1fr; } }
  </style>
</head>
<body>
...
<div class="admin-bar">
  <div class="admin-bar-inner">
    ...
    <span class="admin-badge"><i class="fa-solid fa-users"></i> COMBINED USERS</span>
...
<div class="container">
  ...
  <div class="section-tag">COMBINED_USERS — GROUP LAB</div>
  <h2 style="color:var(--white);margin-bottom:1.5rem;">All Company <span style="color:var(--green);">Users</span></h2>

  <!-- Terminal log -->
  <div class="terminal reveal" style="max-width:700px;margin-bottom:2.5rem;">
    ...
    <div class="t-line"><span class="t-prompt">$</span><span class="t-cmd">SELECT * FROM users; <span style="color:var(--text-dim);">(local PostgreSQL)</span></span></div>
    <div class="t-line"><span class="t-prompt"> </span><span class="t-ok"><?php echo count($companies[0]['users']); ?> users — Paradox Systems</span></div>
    ...
    <div class="t-line"><span class="t-prompt"> </span>
      ...
      <?php else: ?>
        <span class="t-ok"><?php echo count($companies[1]['users']); ?> users — <?php echo htmlspecialchars($companies[1]['company']); ?></span>
      <?php endif; ?>
    </div>
    ...
    <div class="t-line"><span class="t-prompt"> </span>
      ...
      <?php else: ?>
        <span class="t-ok"><?php echo count($companies[2]['users']); ?> users — <?php echo htmlspecialchars($companies[2]['company']); ?></span>
      <?php endif; ?>
    </div>
    <div class="t-line"><span class="t-prompt"> </span><span class="t-out">total: <?php echo $total_users; ?> users merged</span></div>
  </div>
  ...
  <div class="company-cards reveal">
    ...
    <div class="co-card <?php echo $co['color']; ?>">
      ...
      <?php elseif ($co['company_id'] === 'paradox'): ?>
        <div class="co-status ok"><i class="fa-solid fa-database"></i> Local PostgreSQL</div>
      <?php else: ?>
  ...
  <div class="users-wrap reveal d1">
    <table id="usersTable">
      <thead>
        <tr>
          <th>#</th><th>Name</th><th>Email</th><th>Role</th><th>Joined</th><th>Status</th><th>Source</th>
        </tr>
      </thead>
      <tbody>
        <?php $rn = 1; foreach ($all_users as $u):
          $role = $u['role'] ?? 'Standard Member';
          $rc   = str_contains($role,'Enterprise') ? 'enterprise' : (str_contains($role,'Premium') ? 'premium' : 'standard');
        ...
        <tr data-name="<?php echo htmlspecialchars(strtolower($u['name']??'')); ?>"
          ...
          <td style="color:var(--text-dim);">#<?php echo str_pad($rn++,2,'0',STR_PAD_LEFT); ?></td>
        ...
        <tr><td colspan="7" style="text-align:center;padding:3rem;color:var(--text-dim);">No users found. Check DB and CURL endpoints.</td></tr>
        <?php endif; ?>
      </tbody>
    </table>
  </div>
...
function filterTable() {
  ...
  let visible = 0;
  rows.forEach(r => {
    const show = (!q  || r.dataset.name.includes(q) || r.dataset.email.includes(q))
              && (!co || r.dataset.company === co)
              && (!st || r.dataset.status  === st)
              && (!ro || r.dataset.role    === ro);
